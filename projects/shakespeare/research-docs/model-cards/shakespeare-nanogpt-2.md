...
The current best model in the [`shakespeare-nanogpt`](../../README.md) series — the
...
> Both versions and the full story are in [`MODELS.md`](../../MODELS.md); the
...
> [`research-lab/leaderboard.md`](../../research-lab/leaderboard.md).
...
| **Origin** | LLM-assisted research experiment, Round 3 (`research-lab/runs/r3-bpe`) |
| **Architecture** | modern — RoPE, RMSNorm, bias-free (`research-lab/model_modern.py`) |
| **Size** | ~29.9M params |
...
Shakespeare. Prepared as GPT-2 BPE tokens (`research-lab/data/shakespeare_full_bpe/`). A fixed
**250k-character held-out test set** (`research-lab/test.txt`) that no model trains on is
used for evaluation.

## Training procedure

Trained with `research-lab/train_modern.py` on the same Apple Silicon MPS setup as v1
...
*Charts generated by [`dataviz/`](../../dataviz/README.md).*
