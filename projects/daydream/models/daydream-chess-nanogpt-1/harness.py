"""
harness.py (FROZEN release copy, Regular/8x8 only) -- plays this checkpoint
against a skill-limited Fairy-Stockfish opponent, resampling on illegal
moves until one lands or a resample cap forces a random legal move. Doubles
as the verification tool (legal-move rate, clean-completion rate).
...
Regular -- Micro and Grand each have their own frozen folder with their own
...
def play_verification_game(player: DaydreamPlayer, resample_cap: int, opponent_skill: int, rng, model_plays_white: bool):
    engine = Engine(VARIANT, "", skill_level=opponent_skill)
    moves = []
    first_try_legal = 0
