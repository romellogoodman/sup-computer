"""
generate.py — freeze the kenosha-kid-nanogpt-2 corpus (the DRIFT corpus).
...
in-repo. And owning the generator buys the one thing a raw scrape can't:
WEIGHTING. Pynchon's hand-built construals are injected as high-frequency anchors
over the brute-force permutation tail, so a model develops a *preference manifold*
— Pynchon's arrangements crisp, the long tail dim — instead of a flat enumeration.

THE v2 STORY — a self-drifting corpus. v1's converged model spelled the six words
perfectly (near-misses only appeared if you *undertrained*, which also blurred the
anchors: the two dream qualities were coupled). v2 bakes a controlled misspelling
channel into the CORPUS via DRIFT_RATE (default 0.06), applied to the tail
(permutation) lines ONLY — the 9 Pynchon anchors stay pristine and verbatim. Now
a fully-converged (low-loss) model reproduces the anchors crisply AND dreams the
near-misses ("nevver", "Kenoshar"), because the near-misses live in the data, not
in a stopping point. DRIFT_RATE is the dial that decouples them.

Output: raw.txt next to this script (committed, frozen). Deterministic via fixed
seeds, so the corpus regenerates byte-for-byte. (In this frozen release the corpus
is also already vendored as raw.txt; running this overwrites it identically.)
"""
import argparse
import itertools
import os
...
#
# Default 0.06 → the v2 release corpus (drift-r1: crisp anchors, ~33% of lines
# carrying a near-miss, near-zero garble). At DRIFT_RATE=0.0 the corpus
# regenerates BYTE-FOR-BYTE identical to the v1 pristine corpus (drift consumes
# no RNG when the rate is 0). The drift stream uses its OWN derived RNG
# (SEED + 1000) so it is reproducible and independent of the main stream.
DRIFT_RATE = 0.06
...
def main(drift_rate=DRIFT_RATE, out_path=None):
    ...
    # Frozen release: raw.txt lives NEXT TO this script (prepare.py reads it
    # here), not in a data/ subdir — keeps the snapshot self-contained.
    if out_path is None:
        out_path = os.path.join(HERE, "raw.txt")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--drift-rate", type=float, default=DRIFT_RATE,
                    help="per-letter misspelling probability on TAIL lines (anchors stay pristine); v2 default 0.06")
    ap.add_argument("--out", type=str, default=None,
                    help="output path (default: raw.txt next to this script)")
    args = ap.parse_args()
