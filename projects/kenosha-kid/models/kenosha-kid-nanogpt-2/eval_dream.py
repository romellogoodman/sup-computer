"""
eval_dream.py — an automated "dream-score" for kenosha-kid (FROZEN release copy).

This is the self-contained release copy: it imports the vendored sibling model.py
and reads the vendored dataset at ./kenosha/meta.pkl, so it scores THIS snapshot's
checkpoint (ckpt.pt) in place with no project-root or shared-core dependency.
...
Run from inside this folder, e.g.:
  python eval_dream.py --temperature 0.9 --num_samples 40 --print_lines 20
"""
import argparse
import os
import pickle
...
from model import GPTConfig, GPT, CausalSelfAttention  # noqa: E402  vendored sibling
...
def main():
    ...
    ap.add_argument("--out_dir", default=HERE, help="folder with ckpt.pt (default: this folder)")
    ap.add_argument("--data_root", default=HERE, help="root holding <dataset>/meta.pkl (default: this folder)")
    ap.add_argument("--dataset", default=None, help="override; else read from ckpt config, falling back to 'kenosha'")
    ...
    # frozen snapshot ships the dataset in ./kenosha/; fall back there regardless
    # of the dataset name baked into the training config.
    dataset = args.dataset or "kenosha"
    ...
    label = args.label or f"{os.path.basename(args.out_dir.rstrip('/')) or 'kenosha-kid-nanogpt-2'} @ T={args.temperature}{' +MC-dropout' + dp if args.dropout else ''}"
    report(label, s)
    if args.print_lines:
        print("  --- sample lines ---")
