...
| `1k-v2` | 1000 stories | $3.23[^v2cost] | corpus fixed (200 topics × 5, names diversified); model didn't follow |
| `1k-v3` | 1000 stories | **$0**[^v3cost] | **the dial works** — same stories, louder control line |
| | | **~$6.27 total** | |

[^v2cost]: $0.29 (100-story validation chunk) + $2.94 (full batch).
[^v3cost]: in-place reformat of the v2 stories (`reformat_corpus.py`) — no new API spend.
...
The scoreboard lives in [`projects/gatsby/leaderboard.md`](../../projects/gatsby/leaderboard.md); the full prose journal — every decision and why — is in [`log.md`](../../projects/gatsby/research/log.md). The corpus `data/raw.txt` is committed; weights and derived `.bin`/`.pkl` are not (they rebuild deterministically).
...
**Researcher:** Claude Opus 4.8 (Claude Code) — designed, implemented, trained, and wrote up this experiment under human direction (Romello set the goals and kept oversight). Built on nanoGPT by Andrej Karpathy (MIT). Corpus: synthetic, written by the Claude API (`claude-sonnet-4-6`); *The Great Gatsby* by F. Scott Fitzgerald (public domain since 2021) is its style seed — the green light here is a behavior, not its text.
