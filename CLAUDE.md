...
| `tools/` | Researcher tools, not shipped code: `dataviz/` (the chart pipeline — every chart goes through it), `synthgen/` (local-LLM synthetic-corpus pipeline, ADR-0014), `steer/` (shared big-model-steers-small-model layer: OpenAI-compat client + JSON-decision orchestrator, ADR-0026), `linewell/` (line-by-line composition through a small LM, pluggable judge — first instrument: shakespeare-nanogpt-3), `token-chess/` (LLM-vs-LLM benchmark orchestrating Daydream's sampler under a token budget — real-LLM players in progress on a worktree branch) and `claude_cost.py`. |
